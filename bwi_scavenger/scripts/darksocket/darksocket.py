...
# def unpack_int(packet):
#     """Decodes the size of an inbound packet.
#
#     Parameters
#     ----------
#     packet : bytes
#         first packet received in the stream
#
#     Return
#     ------
#     int
...
#     """
#     p = struct.unpack("I", packet[1:5])
#     print p
#     return p[0]
#
#
# def pack_int(size):
#     """Encodes the size of an outbound packet.
#
#     Parameters
#     ----------
#     size : int
...
class Stream:
    ...
    def __init__(self, log=None):
        ...
        self.buffer = bytes()            # Current payload buffer
        ...
        if self.inbound_payload:
            ...
            if len(unpacked[2]) < unpacked[1]:
                ...
                if unpacked[0] == Packet.IMAGE:
                    ...
                    self.handshake = unpacked[2].decode()
...
def client_loop():
    ...
    s.send(pack(Packet.HANDSHAKE, log.name))
    ...
    try:
        while True:
            ...
            if res == StreamEvent.IMAGE:
                ...
                s.send(pack(Packet.DETECTIONS, encode_detections(dets)))
...
def server_example():
    ...
    if res == StreamEvent.HANDSHAKE:
        with open(img_to_detect, "rb") as f:
            image_bytes = f.read()
            s.send(pack(Packet.IMAGE, image_bytes))
    else:
